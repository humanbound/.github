...
This is the **org-wide fallback** security policy for Humanbound projects.
Repositories that ship their own `SECURITY.md` override it; check the
specific repo first if you're reporting against a particular project.

Humanbound takes the security of its open-source software seriously. If you
believe you've found a security vulnerability in any Humanbound repository,
please report it privately using one of the channels below.

## Reporting a vulnerability

**Please do not open a public GitHub Issue.** Report vulnerabilities via
either:
...
- **GitHub Security Advisories:** the "Report a vulnerability" button on the
  affected repository's *Security* tab (private, end-to-end)

A clear report will include:

1. The affected repository and version
...
- Source code in any public repository under
  [github.com/humanbound](https://github.com/humanbound)
- Release artifacts produced by those repositories (PyPI packages, GitHub
  Releases, plugin bundles)
- Build and release pipelines (`.github/workflows/`) in those repositories

Out of scope:

- Vulnerabilities in third-party dependencies (report upstream; we'll track)
- Services at `humanbound.ai` (those have their own disclosure channel —
  see [docs.humanbound.ai/community/](https://docs.humanbound.ai/community/))
...
follow this policy will be acknowledged in release notes unless they opt
out.
